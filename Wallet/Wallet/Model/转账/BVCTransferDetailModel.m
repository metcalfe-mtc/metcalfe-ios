...
-(void)initWithDict:(NSDictionary *)dict{
    ...
    if([dict[@"transactionType"] isEqualToString:@"TrustSet"]){
        self.receiverAccount = dict[@"limitAmount"][@"issuer"];
        if(dict[@"limitAmount"] &&![dict[@"limitAmount"] isKindOfClass:[NSNull class]]){
            if(!dict[@"limitAmount"][@"currency"]){
                self.amount = dict[@"limitAmount"][@"value"];
                self.amount = [self.amount calculateByDividing:@"1000000"];
                self.currency = DEFAULTCURRENCY;
            }else{
                self.amount = dict[@"limitAmount"][@"value"];
                self.currency = dict[@"limitAmount"][@"currency"];
            }
        }
    }else{
